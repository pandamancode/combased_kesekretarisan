   ...
   <div class="col-md-12">
          <div class="box box-success box-solid">
            <div class="box-header with-border">
              ...
              <h3 class="box-title"> Data Pengajuan Surat Keterangan Lulus</h3>
        ...
        <div class="col-sm-12" > 
            ...
            <table id="list-data"  class="table table-bordered table-striped">
              ...
              <tbody>
				        <?php
                  $no=0;
                  foreach($suket_lulus->result() as $res){
                  ...
                  <tr>
                    ...
                    <td  style="padding:5px; text-align:center;" >
                      ...
                      <?php }elseif($res->status == '1'){ ?>
                        <a title="Menunggu Persetujuan" class="btn btn-warning btn-xs btn-detail" data-id="<?php echo $res->id_surat_lulus ?>"><i class="fa fa-eye fa-fw"></i>Lihat Pengajuan</a> 
                      <?php }elseif($res->status == '2'){ ?>
                            <a title="Telah Disetujui" class="btn btn-primary btn-xs btn-konfirmasi" data-id="<?php echo $res->id_surat_lulus ?>"><i class="fa fa-eye fa-fw"></i>Konfirmasi Surat</a> 
                      <?php }elseif($res->status =='3' ){ ?>
                        <a title="Surat Sudah Jadi" data-id="<?php echo $res->id_surat_lulus; ?>" class="btn btn-info btn-xs btn-ambil"><i class="fa fa-check-circle fa-fw"></i>Ambil Surat</a>            
...
$('#form-filter').submit(function (event) {
    ...
    $.ajax({
        type:"POST",
        url:"<?php echo base_url(); ?>TU/SuketLulus/filter_suket",
        data:dataString,
...
$(document).on("click", ".btn-ambil", function() {
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url('TU/SuketLulus/ambil_suket'); ?>",
		data: "id=" +id
	})
	.done(function(data) {
...
$(document).on("click", ".btn-detail", function() {
  ...
  $.ajax({
    method: "POST",
    url: "<?php echo base_url('TU/SuketLulus/detail_suket'); ?>",
    data: "id=" +id
  })
  .done(function(data) {
    $('#tempat-modal').html(data);
...
$(document).on("click", ".showpengambil", function() {
  ...
  $.ajax({
    method: "POST",
    url: "<?php echo base_url('TU/SuketLulus/detail_pengambil'); ?>",
    data: "id=" +id
  })
  .done(function(data) {
    $('#tempat-modal').html(data);
...
$(document).on("click", ".btn-konfirmasi", function() {
  ...
  $.ajax({
    method: "POST",
    url: "<?php echo base_url('TU/SuketLulus/detail_konfirmasi'); ?>",
    data: "id=" +id
  })
  .done(function(data) {
    $('#tempat-modal').html(data);
