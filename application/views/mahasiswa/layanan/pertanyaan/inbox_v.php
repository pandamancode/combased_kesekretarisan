      ...
      <div class="box-body no-padding">
        <ul class="nav nav-pills nav-stacked">
          <li id="inbox"><a href="<?=base_url()?>Mahasiswa/Layanan/pertanyaan/inbox"><i class="fa fa-inbox"></i> Inbox
            <span class="label label-primary pull-right"><?=pertanyaan_reply()?></span></a></li>
      ...
      <div class="box-body no-padding">
        ...
        <div class="table-responsive mailbox-messages">
          <table class="table table-hover table-striped">
            ...
            <?php 
            if($inbox->num_rows()>0){
            foreach($inbox->result() as $r){ ?>
              <tr>
                <td class="mailbox-star"><a href="javascript:;"><i class="fa fa-star <?php if($r->status_baca=='belum'){ echo "text-yellow"; } ?>"></i></a></td>
                <td class="mailbox-name"><a href="<?=base_url()?>Mahasiswa/Layanan/pertanyaan/read/<?=$r->no_pertanyaan?>"><?=$r->nama_staff?></a></td>
                <td class="mailbox-subject"><a href="<?=base_url()?>Mahasiswa/Layanan/pertanyaan/read/<?=$r->no_pertanyaan?>"><b><?=$r->nama_kategori?></b> - klik untuk melihat pesan</a>
                </td>
                <td class="mailbox-date"><?=date('d-m-Y H:i',strtotime($r->tgl_jawab))?> </td>
              </tr>
            <?php } }else{ ?>
              <tr>
